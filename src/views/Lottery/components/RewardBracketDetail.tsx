...
const RewardBracketDetail: React.FC<RewardBracketDetailProps> = ({
  ...
  return (
    <Flex flexDirection="column">
      ...
      <>
        {isLoading || bnbAmount.isNaN() ? (
          <Skeleton my="4px" mr="10px" height={20} width={110} />
        ) : (
          <Balance fontSize="20px" bold unit=" BNB" value={getBalanceNumber(bnbAmount)} decimals={0} />
        )}
        {isLoading || bnbAmount.isNaN() ? (
          <>
          ...
          <Balance
            ...
            color="textSubtle"
            prefix="~$"
            value={getBalanceNumber(bnbAmount.times(bnbPriceBusd))}
            decimals={0}
          />
        )}
        {isHistoricRound && bnbAmount && (
          <>
            {numberWinners !== '0' && (
              <Text fontSize="12px" color="textSubtle">
                {getFullDisplayBalance(bnbAmount.div(parseInt(numberWinners, 10)), 18, 2)} BNB {t('each')}
              </Text>
            )}
            <Text fontSize="12px" color="textSubtle">
