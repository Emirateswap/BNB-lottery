...
const RewardBrackets: React.FC<RewardMatchesProps> = ({ lotteryNodeData, isHistoricRound }) => {
  ...
  useEffect(() => {
    if (lotteryNodeData) {
      const { treasuryFee, amountCollectedInBNB, rewardsBreakdown, countWinnersPerBracket } = lotteryNodeData

      const feeAsPercentage = new BigNumber(treasuryFee).div(100)
      const bnbToBurn = feeAsPercentage.div(100).times(new BigNumber(amountCollectedInBNB))
      const amountLessTreasuryFee = new BigNumber(amountCollectedInBNB).minus(bnbToBurn)
      setState({
        isLoading: false,
        bnbToBurn,
  ...
  const getBNBRewards = (bracket: number) => {
  ...
  const { isLoading, countWinnersPerBracket, bnbToBurn } = state
      ...
      <RewardsInner>
        {rewardBrackets.map((bracketIndex) => (
          <RewardBracketDetail
            ...
            bnbAmount={!isLoading && getBNBRewards(bracketIndex)}
            numberWinners={!isLoading && countWinnersPerBracket[bracketIndex]}
            isHistoricRound={isHistoricRound}
            isLoading={isLoading}
          />
        ))}
        <RewardBracketDetail rewardBracket={0} bnbAmount={bnbToBurn} isBurn isLoading={isLoading} />
      </RewardsInner>
    </Wrapper>
  )
}
