...
const BuyTicketsModal: React.FC<BuyTicketsModalProps> = ({ onDismiss }) => {
  ...
  const [userNotEnoughBNB, setUserNotEnoughBNB] = useState(false)
  ...
  const userBNB = new BigNumber(balance.toString())
  const stringifiedUserBNB = userBNB.toJSON()
  // const memoisedUserCake = useMemo(() => new BigNumber(stringifiedUserCake), [stringifiedUserCake])
  const memoisedUserBNB = useMemo(() => new BigNumber(stringifiedUserBNB), [stringifiedUserBNB])

  // const cakePriceBusd = usePriceCakeBusd()
  const dispatch = useAppDispatch()
  const hasFetchedBalance = fetchStatus === FetchStatus.Fetched
  const userBNBDisplayBalance = getFullDisplayBalance(userBNB, 18, 3)
  ...
  const getTicketCostAfterDiscount = useCallback(
    (numberTickets: BigNumber) => {
      const totalAfterDiscount = priceTicketInBNB
        .times(numberTickets)
        .times(discountDivisor.plus(1).minus(numberTickets))
        .div(discountDivisor)
      return totalAfterDiscount
    },
    [discountDivisor, priceTicketInBNB],
  )

  const getMaxTicketBuyWithDiscount = useCallback(
    (numberTickets: BigNumber) => {
      const costAfterDiscount = getTicketCostAfterDiscount(numberTickets)
      const costBeforeDiscount = priceTicketInBNB.times(numberTickets)
      const discountAmount = costBeforeDiscount.minus(costAfterDiscount)
      const ticketsBoughtWithDiscount = discountAmount.div(priceTicketInBNB)
      const overallTicketBuy = numberTickets.plus(ticketsBoughtWithDiscount)
      return { overallTicketBuy, ticketsBoughtWithDiscount }
    },
    [getTicketCostAfterDiscount, priceTicketInBNB],
  )

  const validateInput = useCallback(
    (inputNumber: BigNumber) => {
      const limitedNumberTickets = limitNumberByMaxTicketsPerBuy(inputNumber)
      const bnbCostAfterDiscount = getTicketCostAfterDiscount(limitedNumberTickets)

      if (bnbCostAfterDiscount.gt(userBNB)) {

        setUserNotEnoughBNB(true)
      } else if (limitedNumberTickets.eq(maxNumberTicketsPerBuyOrClaim)) {
        setMaxTicketPurchaseExceeded(true)
      } else {
        setUserNotEnoughBNB(false)
        setMaxTicketPurchaseExceeded(false)
      }
    },
    [limitNumberByMaxTicketsPerBuy, getTicketCostAfterDiscount, maxNumberTicketsPerBuyOrClaim, userBNB],
  )

  useEffect(() => {
    const getMaxPossiblePurchase = () => {
      const maxBalancePurchase = memoisedUserBNB.div(priceTicketInBNB)
      const limitedMaxPurchase = limitNumberByMaxTicketsPerBuy(maxBalancePurchase)
      let maxPurchase

      // If the users' max BNB balance purchase is less than the contract limit - factor the discount logic into the max number of tickets they can purchase
  ...
  useEffect(() => {
    ...
    const costBeforeDiscount = priceTicketInBNB.times(numberOfTicketsToBuy)
  ...
  }, [ticketsToBuy, priceTicketInBNB, discountDivisor, getTicketCostAfterDiscount])
    ...
    useApproveConfirmTransaction({
      ...
      onConfirm: () => {
        ...
        return callWithGasPrice(lotteryContract, 'buyTickets', [currentLotteryId, ticketsForPurchase], {
          value: getValueAsEthersBn(totalCost).toString()
        })
      },
  ...
  const getErrorMessage = () => {
    if (userNotEnoughBNB) return t('Insufficient BNB balance')
  ...
  return (
    <StyledModal title={t('Buy Tickets')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      /> */}
      <BalanceInput
        isWarning={account && (userNotEnoughBNB || maxTicketPurchaseExceeded)}
        placeholder="0"
        value={ticketsToBuy}
        onUserInput={handleInputChange}
        currencyValue={
          userBNB &&
          `~${ticketsToBuy ? getFullDisplayBalance(priceTicketInBNB.times(new BigNumber(ticketsToBuy))) : '0.00'} BNB`
        }
      />
      <Flex alignItems="center" justifyContent="flex-end" mt="4px" mb="12px">
        <Flex justifyContent="flex-end" flexDirection="column">
          {account && (userNotEnoughBNB || maxTicketPurchaseExceeded) && (
      ...
      <Flex flexDirection="column">
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text color="textSubtle" fontSize="14px">
            {priceTicketInBNB && getFullDisplayBalance(priceTicketInBNB.times(ticketsToBuy || 0))} BNB
