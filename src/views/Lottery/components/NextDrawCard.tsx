...
const NextDrawCard = () => {
  ...
  const { endTime, amountCollectedInBNB, userTickets, status } = currentRound
  ...
  const getPrizeBalances = () => {
    if (status === LotteryStatus.CLOSE || status === LotteryStatus.CLAIMABLE) {
      return (
        <Heading scale="xl" color="#1FC7D4" textAlign={['center', null, null, 'left']}>
          {t('Calculating')}...
        </Heading>
      )
    }
      ...
      <>
        ...
        {prizeInBusd.isNaN() ? (
          ...
          <Balance
            ...
            textAlign={['center', null, null, 'left']}
            unit=" BNB"
            value={getBalanceNumber(amountCollectedInBNB)}
            decimals={0}
          />
        )}
      </>
