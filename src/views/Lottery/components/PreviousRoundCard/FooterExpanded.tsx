...
const PreviousRoundCardFooter: React.FC<{ lotteryNodeData: LotteryRound; lotteryId: string }> = ({
  ...
  if (lotteryNodeData) {
    const { amountCollectedInBNB } = lotteryNodeData
    prizeInBusd = amountCollectedInBNB.times(bnbPriceBusd)
  }
      ...
      <>
        ...
        {prizeInBusd.isNaN() ? (
          ...
          <Balance
            ...
            color="textSubtle"
            unit=" BNB"
            value={getBalanceNumber(lotteryNodeData?.amountCollectedInBNB)}
            decimals={0}
          />
        )}
      </>
