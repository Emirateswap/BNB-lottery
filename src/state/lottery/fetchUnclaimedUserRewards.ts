...
const fetchBNBRewardsForTickets = async (
  winningTickets: LotteryTicket[],
): Promise<{ ticketsWithUnclaimedRewards: LotteryTicket[]; bnbTotal: BigNumber }> => {
  ...
  try {
    const bnbRewards = await multicallv2<any>(lotteryV2Abi, calls)

    const bnbTotal = bnbRewards.reduce((accum: BigNumber, bnbReward: ethers.BigNumber[]) => {
      return accum.plus(new BigNumber(bnbReward[0].toString()))
    }, BIG_ZERO)

    const ticketsWithUnclaimedRewards = winningTickets.map((winningTicket, index) => {
      return { ...winningTicket, bnbReward: bnbRewards[index] }
    })
    return { ticketsWithUnclaimedRewards, bnbTotal }
  } catch (error) {
    console.error(error)
    return { ticketsWithUnclaimedRewards: null, bnbTotal: null }
  }
}
...
export const getWinningTickets = async (
  ...
  if (unclaimedWinningTickets.length > 0) {
    const { ticketsWithUnclaimedRewards, bnbTotal } = await fetchBNBRewardsForTickets(unclaimedWinningTickets)
    return { ticketsWithUnclaimedRewards, allWinningTickets, bnbTotal, roundId }
  }

  if (allWinningTickets.length > 0) {
    return { ticketsWithUnclaimedRewards: null, allWinningTickets, bnbTotal: null, roundId }
  }
